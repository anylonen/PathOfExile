# Buildit

> Tähän dokumenttiin on kerätty eri buildeihin liittyvää materiaalia

## Spark Archmage Hierophant

[Spark Archmage Hierophant](https://mobalytics.gg/poe/builds/bigducks-spark-archmage-heirophant)

## Holy Flame Totem Hierophant]

[Holy Flame Totem Hierophant](https://maxroll.gg/poe/build-guides/holy-flame-totem-hierophant-league-starter)

### Gear
#### Early Maps
[Wand (+1 Fire skill, spell dam, cast speed)](https://www.pathofexile.com/trade/search/Mirage/BgJ3EG3wU8)
[Crested Tower Shield (chance to block, max life)](https://www.pathofexile.com/trade/search/Mirage/Z65DP5LztQ)
[Onyx Amulet (+fire skill, strength, resistances)](https://www.pathofexile.com/trade/search/Mirage/yYRwrXrBHR)
[Heavy Belt (strength, max life, resistances)](https://www.pathofexile.com/trade/search/Mirage/Ogw6pmnVFE)
[Amethyst Ring (max life, cast speed, resistances)](https://www.pathofexile.com/trade/search/Mirage/V5EnMlm2hp)
[Titan Greaves (movement speed, max life, life regen, resistances)](https://www.pathofexile.com/trade/search/Mirage/5nkMoobnta)
[Titan Gauntlets (max life, armour, life regen, resistances)](https://www.pathofexile.com/trade/search/Mirage/8r2Ba40BUV)
[Ezomyte Burgonet (max life, armour, life regen, resistances)](https://www.pathofexile.com/trade/search/Mirage/Ogw6qPvgHE)
[Astral Plate (max life, armour, life regen, resistances)](https://www.pathofexile.com/trade/search/Mirage/X3DMwG67uP)

[Pyroclast Mine of Sabotage](https://www.pathofexile.com/trade/search/Mirage/wvl0w5Wmcb)

## Siege Ballista Hierophant (3.27)

### Plussat ja miinukset

#### Plussat

+ Works great on a budget (great league starter too!)
+ Fast T16 and T16.5 mapper (can do all in-atlas content)
+ Lazy play (ballistas target and kill enemies for you; this is a one-button build)
+ Good defenses (once the build is fully done!)
+ Halpoja uniikkeja valtaosa


#### Miinukset

- Cannot farm pinnacle bosses on Uber Mode difficulty
- This build was not designed for T17 mapping
- Uniikkeja pakko hommata

### Guide

[POEGuy - Siege Ballista Hierophant](https://www.poe-vault.com/guides/siege-ballista-hierophant-build-guide)

#### Yleiset virheet (Guidesta poimittu)

- Not taking Dexterity on rare items (most common problem).
- Not having 20% quality on the Siege Ballista skill gem (this is a significant DPS loss!).
- Using a low-level Siege Ballista skill gem. Minimi lvl 15 ja Quality 20%
  - If you use a low-level gem, you deal Low DPS, and your totems die fast.
- Not unlocking the labs.
  - You should not enter the mapping phase without completing the first three labs.
  - There’s a big reason we picked Hierophant for this build concept!
- Not taking the “Point Blank” Keystone / not using a “Sleepless Sentries” Medium Cluster Jewel.
 - Tip: Buy one with 4 or 5 passives – it will save you 1 skill point!
- Not using the right flasks / Not using the bow setup correctly (to summon the Golem and curse with Frostbite).
- Not having elemental resistances capped during the mapping phase.

#### EA Ballista -> Siege Ballista -transformaatio

- Level minimissään noin 78
- Dexterity minimissään 1000
- Siege Ballista -skill gem L15/Q20 (**HUOMI! Osta Siosalta Ac3:ssa** ja laita offhandiin tms. levelautumaan)

### Path of Building

* 3.27: [Levelaus 1 - 78 (EA Ballista)](https://pobb.in/wF5R9WdVeJf1)
* 3.27: [Budjetti ja normaali lvl 99](https://pobb.in/wXVStDuZrqHX)
  - HUOM! Skillipuussa stepit tasoille 78, 87 ja 98

### Item filter

* [SiegeBallistaHierophant](https://www.filterblade.xyz/?profile=Anylo&saveState=MH9SYAOS011LUF&isPreset=false&game=Poe1)
* [Anylo's Item Filters - SiegeBallistaHierophant](https://www.pathofexile.com/account/view-profile/Anylo-2720/item-filters)

### Craftaus

### Kallis versio

- [Shrapnel Ballista](https://www.poe-vault.com/guides/shrapnel-ballista-hierophant-build-guide)
